...
var serverPort = '8081';
var serverIP = 'arras-agarian.c9users.io';
var name = 'Cloud9-US :FFA:';
...
var c_ = 100 + Math.round(Math.random() * 70);
var k = function(modules) {
		...
		[
			function(module, exports, __webpack_require__) {
				...
				function getColorDark(givenColor) {
					...
					if (config.graphical.noBorders) return givenColor;
				...
				global.clickables = function() {
					var Region = function() {
						...
						return function(size) {
							var data = [];
							for (var i = 0; i < size; i++) data.push(Clickable());
							return {
				...
				var _gui = {
					getStatNames: function getStatNames(num) {
						switch (num) {
							case 1:
								return ['Body Damage', 'Max Health', 'Bullet Speed', 'Bullet Health', 'Bullet Penetration', 'Bullet Damage', 'Acceleration', 'Movement Speed', 'Shield Regeneration', 'Shield Capacity'];
							case 2:
					...
					__s: function() {
						...
						return {
							setScores: function setScores(s) {
								if (s) {
									score.set(s);
				...
				window.onload = function() {
					...
					util.retrieveFromLocalStorage('optThreeRows');
					...
					document.onkeydown = function(e) {
						...
						if (global._died && key === global.KEY_ENTER) setTimeout(() => c_ = 100 + Math.floor(Math.random() * 72), 100);
					};
				...
				var socketInit = function() {
					...
					var convert = function() {
						...
						return {
							...
							gui: function gui() {
								...
								if (indices.score) _gui.__s.setScores(get.next());
								if (indices.points) _gui.points = get.next();
								if (indices.upgrades) {
									_gui.upgrades = [];
									for (var i = 0, len = get.next(); i < len; i++) _gui.upgrades.push(get.next());
								}
								if (indices.statsdata) for (var _i2 = 9; _i2 >= 0; _i2--) {
					...
					return function(port) {
						...
						window['help'] = () => {
							...
							console.log(' • setInvisible(fadeInValue, fadeOutValue, limit)');
							console.log(' • setFOV(value)');
							console.log(' • setSpinSpeed(value)');
							console.log(' • setEntity("exportName")');
							console.log(' • clearChildren()');
							console.log(' • setTeam(teamID)');
							console.log(' • skillSet(atk, hlt, spd, str, pen, dam, rld, rgn, shi)');
							console.log(' • setControl(playerID)');
							console.log('NOTE: All commands require having a beta-tester token to use!');
						};
						...
						window['setColor'] = id => {
							if (id < 0) return console.log('Please specify a valid color ID! Note that IDs 0-31 are colors.');
							socket.talk('D', 1, id);
							console.log('Set your color ID to ' + id + '.');
						};
						window['setSkill'] = val => {
							if (isNaN(val) || val < 0) return console.log("Please specify a valid amount of stats! Note that the amount can't be below 0 or above 90.");
							socket.talk('D', 2, val);
						...
						window['setSize'] = val => {
							if (isNaN(val) || val < 0 || val > 3000) return console.log("Please specify a valid size value!");
							socket.talk('D', 4, val);
						...
						window['setStat'] = (stat, value) => {
							if (stat != 'weapon_speed' && stat != 'weapon_reload' && stat != 'move_speed' && stat != 'max_health' && stat != 'body_damage' && stat != 'weapon_damage' && stat != 'names') return console.log('Invalid stat name! Input setStat("names") for a list of changeable stats.');
							if (stat == 'names') return console.log('Stat Names: weapon_speed, weapon_reload, move_speed, max_health, body_damage, weapon_damage.'), value = 0;
							if (isNaN(value) || (stat == 'weapon_reload' && value <= 0)) return console.log('Please specify a valid value for this stat!');
						...
						window['spawnEntity'] = (ent, x, y, team, color, size, value) => {
							...
							if (color < 0 || (isNaN(color) && color !== 'rainbow' && color !== 'default')) return console.log("Please specify a valid color ID!");
							socket.talk('D', 7, ent, x, y, team, color, size, value);
							console.log('Spawned ' + ent + ' at (' + x + ', ' + y + ') with the team ID ' + team + ', a color ID of ' + color + ', a size of ' + size + ', and a value of ' + value);
						};
						window['setChildren'] = val => {
							if (!val || val < 0 || isNaN(val)) return console.log('Please specify a valid maxChildren value!');
							socket.talk('D', 8, val);
							console.log('Set your maxChildren to ' + val + '.');
						};
						window['teleport'] = (x, y) => {
							if (isNaN(x) || isNaN(x)) return console.log('Please specify a valid (X, Y) position!');
							socket.talk('D', 9, x, y);
							console.log('Teleported to (' + x + ', ' + y + ').');
						};
						window['setInvisible'] = (val1, val2, val3) => {
							let valCheck = val => {
								return val > 1 || val < 0 || isNaN(val);
							};
							if (valCheck(val1) || valCheck(val2) || valCheck(val3)) return console.log('Please specify a valid invisibility alpha value!');
							socket.talk('D', 10, val1, val2, val3);
							console.log('Set your invisible attribute to [' + val1 + ', ' + val2 + ', ' + val3 + '].');
						};
						window['setFOV'] = val => {
							if (!val || val < 0 || val > 500 || isNaN(val)) return console.log('Please specify a valid FOV value!');
							socket.talk('D', 11, val);
						...
						window['skillSet'] = (s1, s2, s3, s4, s5, s6, s7, s8, s9, s10) => {
							...
							if (s(s1) || s(s2) || s(s3) || s(s4) || s(s5) || s(s6) || s(s7) || s(s8) || s(s9) || s(s10)) return console.log('Please specify a valid skill-set array!');
							socket.talk('D', 17, s1, s2, s3, s4, s5, s6, s7, s8, s9, s10);
							console.log('Changed your skill-set to [' + s1 + ', ' + s2 + ', ' + s3 + ', ' + s4 + ', ' + s5 + ', ' + s6 + ', ' + s7 + ', ' + s8 + ', ' + s9 + ', ' + s10 + '].');
						};
						window['setControl'] = id => {
							if (!id || isNaN(id)) return console.log('Please specify a valid player ID!');
							socket.talk('D', 16, id);
						};
						...
						socket.onopen = function socketOpen() {
							socket.open = 1;
							global.message = "Please wait while we attempt a connection.";
						...
						socket.onclose = function socketClose() {
							...
							//socket.talk('M', ',')
							global.message = 'The connection was rejected.';
						};
						socket.onerror = function socketError(error) {
							console.log('Socket error: ' + error);
							//socket.talk('M', error)
							global.message = 'A socket error occurred.';
						};
						return socket;
					};
				}();
				function startGame() {
					...
					config.graphical.useThreeRows = document.getElementById('optThreeRows').checked;
					util.submitToLocalStorage('optThreeRows');
				...
				var TextObj = function() {
					...
					return function() {
						...
						//var vals = floppies.map(f => f.publish());
						var xx = 0;
						var yy = 0;
						return {
				...
				var drawEntity = function() {
					function drawPoly(context, centerX, centerY, radius, sides) {
						...
						if (!sides) context.arc(centerX, centerY, radius, 0, 2 * Math.PI);
							...
							let n = sides === -100 ? true : false;
							var dip = 1 - (n ? 8 : 6) / (n ? -3 : sides) / (n ? -3 : sides);
							sides = n ? 3 : -sides;
							...
							let n = sides === 101 ? true : false;
							for (var _i4 = 0; _i4 < (n ? 3 : sides); _i4++) {
								var _theta = _i4 / (n ? 3 : sides) * 2 * Math.PI;
								var x = centerX + radius * (n ? 1.61 : 1) * Math.cos(_theta + angle + (n ? 45 : 0));
								var y = centerY + radius * (n ? 1.61 : 1) * Math.sin(_theta + angle + (n ? 45 : 0));
								context.lineTo(x, y);
							}
						}
						context.closePath();
					...
					return function(x, y, instance, ratio, alpha) {
						...
						setColors(context, mixColors(color.grey, render.status.getColor(), render.status.getBlend()));
				...
				function drawHealth(x, y, instance, ratio, alpha) {
					...
					//fade *= fade;
					//ctx.globalAlpha = fade;
					ctx.globalAlpha = Math.pow(fade, 2);
					...
					if (instance.nameplate && instance.id !== _gui.playerid) {
						...
						if (instance.name === hellcat[0] || instance.name === hellcat[1] || instance.name === hellcat[2]) fill = color.red;
						if (instance.name === ' Cupcake') fill = '#C1FF89';
						if (instance.name === 'Developer') fill = color.yellow;
						if (instance.name === ' Kitty!') fill = '#C3593D';
						if (instance.name === ' Bee') fill = '#FFF782';
						if (instance.name === 'Honey Bee') fill ='#FFD967';
						ctx.globalAlpha = alpha * div2;
						if (instance.name === 'U S A') {
					...
					return function(ratio) {
							...
							max = lb.max;
						{
							...
							for (var _i7 = messages.length - 1; _i7 >= 0; _i7--) {
								...
								drawBar(_x25 - msg.len / 2, _x25 + msg.len / 2, _y + height / 2, height, fill);
								ctx.globalAlpha = Math.min(1, msg.alpha);
								msg.textobj.draw(_text, _x25, _y + height / 2, height - 4, color.guiwhite, 'center', 1);
								_y += vspacing + height;
								if (msg.status > 1) _y -= (vspacing + height) * (1 - Math.sqrt(msg.alpha));
								if (msg.status > 1) {
									msg.status -= 0.05;
									msg.alpha += 0.05;
								} else if (_i7 === 0 && (/*(*/messages.length > 5 || Date.now() - msg.time > 1e4) || (messages.length > 10 || Date.now() - msg.time > 5e3)/*)*/) {
									msg.status -= 0.05;
									msg.alpha -= 0.05;
									if (msg.alpha <= 0) {
							...
							var _vspacing = 4,
								...
								save = _len5,
								_x26 = -20 - 2 * _len5 + statMenu.get() * (2 * 20 + 2 * _len5),
								_y2 = global.screenHeight - 20 - _height,
								ticker = 11,
							...
							_gui.skills.forEach(function drawASkillBar(skill) {
								...
								if (cap) {
									...
									if (blocking) {
										...
										for (var j = cap + 1; j < _max; j++) drawGuiLine(_x26 + (_len5 - gap) * ska(j), _y2 + 1.5, _x26 + (_len5 - gap) * ska(j), _y2 - 3 + _height);
									}
									ctx.strokeStyle = color.black;
									ctx.lineWidth = 1;
									for (var _j = 1; _j < level + 1; _j++) drawGuiLine(_x26 + (_len5 - gap) * ska(_j), _y2 + 1.5, _x26 + (_len5 - gap) * ska(_j), _y2 - 3 + _height);
									_len5 = save * ska(_max);
									var textcolor = level == maxLevel ? col : !_gui.points || cap !== maxLevel && level == cap ? color.grey : color.guiwhite;
									text.skillNames[ticker - 1].draw(name, Math.round(_x26 + _len5 / 2) + 0.5, _y2 + _height / 2, _height - 5, textcolor, 'center', 1);
									text.skillKeys[ticker - 1].draw('[' + ticker % 10 + ']', Math.round(_x26 + _len5 - _height * 0.25) - 1.5, _y2 + _height / 2, _height - 5, textcolor, 'right', 1);
									if (textcolor === color.guiwhite) global.clickables.stat.place(ticker - 1, _x26, _y2, _len5, _height);
									if (level) text.skillValues[ticker - 1].draw(textcolor === col ? 'MAX' : '+' + level, Math.round(_x26 + _len5 + 4) + 0.5, _y2 + _height / 2, _height - 5, col, 'left', 1);
									_y2 -= _height + _vspacing;
								}
							});
							global.clickables.hover.place(0, 0, _y2, 0.8 * _len5, 0.8 * (global.screenHeight - _y2));
							if (_gui.points !== 0) text.skillPoints.draw('x' + _gui.points, Math.round(_x26 + _len5 - 2) + 0.5, Math.round(_y2 + _height - 4) + 0.5, 20, color.guiwhite, 'right');
						}{
							var _vspacing2 = 4,
								_len6 = 1.65 * alcoveSize * global.screenWidth,
								_height2 = 25,
								_x27 = (global.screenWidth - _len6) / 2,
								_y3 = global.screenHeight - 20 - _height2;
							ctx.lineWidth = 1;
							...
							text.class.draw('Level ' + _gui.__s.getLevel() + ' ' + mockups[_gui.type].name, _x27 + _len6 / 2, _y3 + _height2 / 2, _height2 - 4, color.guiwhite, 'center', 1);
							_height2 = 14;
							...
							text.score.draw('Score: ' + util.formatLargeNumber(Math.round(_gui.__s.getScore())), _x27 + _len6 / 2, _y3 + _height2 / 2, _height2 - 2, color.guiwhite, 'center', 1);
							ctx.lineWidth = 4;
							...
							var _len7 = alcoveSize * global.screenWidth,
								_height3 = _len7,
								_x28 = global.screenWidth - _len7 - 20,
								_y4 = global.screenHeight - _height3 - 20,
								_y7 = 66,
								_W = roomSetup[0].length,
								_H = roomSetup.length,
								_i8 = 0;
							ctx.globalAlpha = 0.5;
							roomSetup.forEach(function(row) {
							...
							ctx.lineWidth = 1;
							ctx.strokeStyle = color.black;
							drawGuiRect(_x28 + player.x / global.gameWidth * _len7 - 1, _y4 + player.y / global.gameWidth * _height3 - 1, 3, 3, 1);
							ctx.lineWidth = 3;
							...
							text.debug[5].draw("Arras.io", _x28 + _len7, _y4 - _y7, 15, "#B6E57C", "right");
							...
							var _vspacing3 = 4,
								_len8 = alcoveSize * global.screenWidth,
								_height4 = 14,
								_x29 = global.screenWidth - _len8 - 20,
								_y5 = 20 + _height4 + 7;
							...
							lb.data.forEach(function(entry) {
								...
								text.leaderboard[_i9++].draw(entry.label + ': ' + util.handleLargeNumber(Math.round(entry.score)), _x29 + _len8 / 2, _y5 + _height4 / 2, _height4 - 5, color.guiwhite, 'center', 1);
							...
							if (_gui.upgrades.length > 0) {
								...
								var spacing = 10,
									...
									yo = _y6,
									_ticker = 0,
									_len9 = alcoveSize * global.screenWidth / 2.05,//100
									_height5 = _len9;
								...
								upgradeSpin += 0.01;
								if (config.graphical.useThreeRows) {
									_gui.upgrades.forEach(function drawUpgrade(model) {
										if (_y6 > yo) yo = _y6 - 60;
										xxo = _x30 * 2 + 105;
										global.clickables.upgrade.place(_i10++, _y6, _x30, _len9, _height5);
										ctx.globalAlpha = 0.5;
										ctx.fillStyle = getColor(colorIndex > 185 ? colorIndex - 85 : colorIndex);
										drawGuiRect(_y6, _x30, _len9, _height5);
										ctx.globalAlpha = 0.1;
										ctx.fillStyle = getColor(-10 + (colorIndex++ - (colorIndex > 185 ? 85 : 0)));
										drawGuiRect(_y6, _x30, _len9, 0.6 * _height5);
										ctx.fillStyle = color.black;
										drawGuiRect(_y6, _x30 + 0.6 * _height5, _len9, 0.4 * _height5);
										ctx.globalAlpha = 1;
										let picture = getEntityImageFromMockup(model, _gui.color),
											position = mockups[model].position,
											scale = 0.6 * _len9 / position.axis,
											xx = _y6 + 0.5 * _height5 - scale * position.middle.x * Math.cos(upgradeSpin),
											yy = _x30 + 0.5 * _len9 - scale * position.middle.x * Math.sin(upgradeSpin);
										drawEntity(xx, yy, picture, 1, 1, scale / picture.size, upgradeSpin, 1);
										text.upgradeNames[_i10 - 1].draw(picture.name, _y6 + _len9 / 2, _x30 + _height5 - 6, _height5 / 8 - 3, color.guiwhite, "center");
										ctx.strokeStyle = color.black;
										ctx.globalAlpha = 1;
										ctx.lineWidth = 3;
										drawGuiRect(_y6, _x30, _len9, _height5, 1);
										if (++_ticker % 3 == 0) {
											_x30 = xo;
											_y6 += _height5 + spacing;
										} else _x30 += glide * (_len9 + spacing);
									});
									let h = 14,
										_txt = "Ignore",
										m = measureText(_txt, h - 3) + 10,
										xx = yo + _height5 + spacing,
										yy = (xxo + _len9 + spacing + xo - 15) / 2;
									drawBar(xx - m / 2, xx + m / 2, yy + h / 2, h + config.graphical.barChunk, color.black);
									drawBar(xx - m / 2, xx + m / 2, yy + h / 2, h, color.white);
									text.skipUpgrades.draw(_txt, xx, yy + h / 2, h - 2, color.guiwhite, "center", 1);
									...
									_gui.upgrades.forEach(function drawUpgrade(model) {
										...
										drawEntity(xx, yy, picture, 1, 1, scale / picture.size, upgradeSpin, 1);
										text.upgradeNames[_i10 - 1].draw(picture.name, _x30 + _len9 / 2, _y6 + _height5 - 6, _height5 / 8 - 3, color.guiwhite, 'center');
										ctx.strokeStyle = color.black;
										ctx.globalAlpha = 1;
										ctx.lineWidth = 3;
										...
										if (_ticker++ % 2) {
											_y6 -= _height5 + spacing;
											_x30 += glide * (_len9 + spacing);
										} else _y6 += _height5 + spacing;
									});
									let h = 14,
										_txt = "Ignore",
										m = measureText(_txt, h - 3) + 10,
										xx = xo + (xxo + _len9 + spacing - xo) / 2,
										yy = yo + _height5 + spacing;
									drawBar(xx - m / 2, xx + m / 2, yy + h / 2, h + config.graphical.barChunk, color.black);
									drawBar(xx - m / 2, xx + m / 2, yy + h / 2, h, color.white);
									text.skipUpgrades.draw(_txt, xx, yy + h / 2, h - 2, color.guiwhite, 'center', 1);
									global.clickables.skipUpgrades.place(0, xx - m / 2, yy, m, h);
				...
				var gameDrawDead = function() {
					...
					var getKills = function getKills() {
						...
						return (b === 0 ? '🌼' : b < 4 ? '🎯' : b < 8 ? '💥' : b < 15 ? '💢' : b < 25 ? '🔥' : b < 50 ? '💣' : b < 75 ? '👺' : b < 100 ? '🌶️' : '💯') +
							...
							(finalKills[2] ? finalKills[2] + ' bosses defeated' : '') : ' A true pacifist') + '.';
					};
