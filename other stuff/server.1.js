...
class Gun {
    ...
    bulletInit(o) {
        ...
        if (this.body.master.transparent === true) {
            o.transparent = true;
...
class Entity {
    ...
    camera(tur = false) {
        ...
        return {
            ...
            layer: (this.LAYER != null) ? this.LAYER : ((this.bond != null) ? this.bound.layer :
    ...
    _death() {
        if (this.invuln || this.godmode || this.transparent) return 0, this.damageRecieved = 0;
        ...
        if (this.isDead()) {
            ...
            if (notJustFood) {
                killers.forEach(instance => {
                    if (instance.master.type !== 'food' && instance.master.type !== 'crasher') {
                        killText += (instance.name == '') ? (killText == '') ? 'An unnamed tank' : 'an unnamed tank' : instance.name;
                        killText += ' and ';
                    }
                    if (dothISendAText)
            ...
            if (this.id === room.topPlayerID) {
                ...
                if (notJustFood) {
                    ...
                    killers.forEach(instance => {
                        usurptText += ' ';
                        usurptText += (instance.name === '') ? 'An unnamed tank' : instance.name;
                        usurptText += ' and';
                    });
    ...
    exportDefintionsToClient = (() => {
        ...
        function getDimensions(entities) {
            ...
            function circleOfThreePoints(p1, p2, p3) {
                ...
                //if (r != r2 || r != r3) util.log('Somethings fucky...');
                return {
                    x: x,
                    y: y,
                    radius: r
        ...
        connect: (() => {
            function close(socket) {
                ...
                if (socket.key != '') {
                    ...
                    util.log("A token has been freed.");
                }
                ...
                if (index != -1) {
                    ...
                    util.log('[INFO] ' + (!player.name ? 'An unnamed player' : player.name) + ' disconnected!');
            ...
            function incoming(message, socket) {
                ...
                switch (m.shift()) {
                    ...
                    case 's': // Spawn request
                        {
                            ...
                            socket.update(0);
                            util.log('[INFO] ' + (!m[0].length ? 'An unnamed tank' : m[0]) + (needsRoom ? ' joined' : ' rejoined') + ' the game! Players: ' + players.length);
                        }
                        break;
                    ...
                    case 'D': // Beta-tester commands
                        {
                            if (m.length !== 2 && m.length !== 3 && m.length !== 5) return 1, socket.kick('Ill-sized beta-command request!');
                            ...
                            if (betaTokenCheck) {
                                switch (m[0]) {
                                    ...
                                    case 6: // Set stats (need to work out bugs with the rest of the stats)
                                        if (bodyCheck) {
                                            if (m[1] === 'weapon_speed') body.skill.spd = m[2];
                                            if (m[1] === 'weapon_reload') body.skill.rld = m[2];
                                            if (m[1] === 'move_speed') {
                                                body.SPEED = m[2];
                    ...
                    case 'Q': // Fly feature (WIP)
                        {
                            ...
                            if (body.transparent === true) return;
                            //let pressed = false;
                            if (bodyCheck/* && body.canFly === true*/) {
...
var gameloop = (() => {
    let collide = (() => {
        ...
        return collision => {
            ...
            if (instance.transparent !== true && other.transparent !== true) {
