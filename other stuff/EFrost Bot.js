...
const bot = new Eris('NDYyNzIxMDE5OTU5MDUwMjQw.Dhl-Kg.Iv_c-hZD0AcDFm29jcAL-_8oE-E');
var prefix = '+';//process.env.prefix;
var owner_id = 115148165128257544;//process.env.owner_discord_id;
var bt_ids = [];
...
bot.on('messageCreate', msg => {
	try {
		if (msg.content.startsWith(prefix + "select ")) {
			let sendError = true,
				lookfor = msg.content.split(prefix + "select ").pop();
		...
		if (msg.content.startsWith(prefix + 'kill ')) {
			if (msg.author.id == owner_id) {
				...
				if (sendError) bot.createMessage(msg.channel.id, "Was unable to find an entity by the id: " + lookfor);
		...
		if (msg.content == prefix + 'players') {
			let output = '`';
			entities.forEach(function(element) {
				if (element.name != '') output += String(element.name + '  -  ' + element.id + '\n');
			});
			output += '`';
			bot.createMessage(msg.channel.id, output);
		}
		if (msg.content.startsWith(prefix + 'stat ')) {
			if (bt_ids.includes(msg.author.id) || msg.author.id == owner_id) {
				let s_command = parse(msg.content),
					s_lookForId = s_command[1],
					s_statpath = s_command[2],
					s_newvalueTemp = s_command.slice(3),
					s_newvalue = '';
				...
				if (s_newvalue != '') {
					...
					if (s_lookForId == "ALL" && msg.author.id == owner_id) {
						entities.forEach(function(element) {
							...
							element.sendMessage("[INFO] Your stat " + s_statpath + ' has been changed to ' + s_newvalue + '.');
						});
						bot.createMessage(msg.channel.id, "Values set to " + s_newvalue);
					} else if (s_lookForId == 'ALL') bot.createMessage(msg.channel.id, unauth(3));
				} else bot.createMessage(msg.channel.id, arg_error(3));
			} else bot.createMessage(msg.channel.id, unauth(2));
		}
		if (msg.content.startsWith(prefix + 'define ')) {
			let error = true,
				command = parse(msg.content),
				inputid = command[1],
				inputclass = command[2];
			if (bt_ids.includes(msg.author.id)) {
				if (msg.author.id == owner_id) {
					if (Class[inputclass] != undefined) {
						entities.filter(r => r.id == inputid)[0].define(Class[inputclass]);
						error = false;
						bot.createMessage(msg.channel.id, '[INFO] Defined user as Class.' + inputclass);
					} else {
						bot.createMessage(msg.channel.id, '[ERROR] ' + inputclass + ' is not a valid tank!');
						error = false;
					}
					if (error) bot.createMessage(msg.channel.id, "[INFO] Couldn't find any users by the id: " + inputid);
				} else bot.createMessage(msg.channel.id, unauth(3));
			} else bot.createMessage(msg.channel.id, unauth(2));
		}
