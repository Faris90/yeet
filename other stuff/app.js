...
var submitToLocalStorage = name => {
    localStorage.setItem(name + 'Value', document.getElementById(name)
        .value);
    localStorage.setItem(name + 'Checked', document.getElementById(name)
        .checked);
    return false;
};
var retrieveFromLocalStorage = name => {
    ...
    document.getElementById(name)
        .checked = localStorage.getItem(name + 'Checked') === 'true';
    return false;
};
var handleLargeNumber = (a, cullZeroes = false) => {
    ...
    if (a < Math.pow(10, 12)) {
        return (a / Math.pow(10, 9))
            .toFixed(2) + "b";
    }

    if (a < Math.pow(10, 15)) {
        return (a / Math.pow(10, 12))
            .toFixed(2) + "t";
    }

    return (a / Math.pow(10, 15))
        .toFixed(2) + "q";
...
window.onload = () => {
    ...
    document.getElementById('serverName')
        .innerHTML = '<h4 class="nopadding">' + serverName + '</h4>';
    // Save forms
    ...
    // Set default theme
    if (document.getElementById('optColors')
        .value === '') {
        document.getElementById('optColors')
            .value = 'normal';
    }
    if (document.getElementById('optBorders')
        .value === '') {
        document.getElementById('optBorders')
            .value = 'normal';
    }
    // Game start stuff
...
function startGame() {
    ...
    config.graphical.screenshotMode = document.getElementById('optScreenshotMode')
        .checked;
    submitToLocalStorage('optFancy');
    config.graphical.pointy = !document.getElementById('optNoPointy')
        .checked;
    submitToLocalStorage('optNoPointy');
    config.graphical.fancyAnimations = !document.getElementById('optFancy')
        .checked;
    submitToLocalStorage('optPredictive');
    config.lag.unresponsive = document.getElementById('optPredictive')
        .checked;
    submitToLocalStorage('optBorders');
    switch (document.getElementById('optBorders')
        .value) {
        case 'normal':
    ...
    let a = document.getElementById('optColors')
        .value;
    ...
    global.playerKey = playerKeyInput.value.replace(/(<([^>]+)>)/ig, '')
        .substring(0, 64);
...
const gameDraw = (() => {
    ...
    return ratio => {
        ...
        { // Draw minimap and FPS monitors
            ...
            text.debug[1].draw(
                'Server Speed: ' + (100 * gui.fps)
                .toFixed(2) + '%' + ((gui.fps === 1) ? '' : ' OVERLOADED!'),
                x + len, y - 50 - 1 * 14,
